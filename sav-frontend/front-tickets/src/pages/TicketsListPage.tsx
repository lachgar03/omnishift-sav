...
export default function TicketsListPage({ scope = 'all' }: TicketsListPageProps) {
  ...
  const isMyTicketsScope = scope === 'my' || (!isAdmin && !isTechnician)

  // Use different API endpoints based on scope and role
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['tickets', page, pageSize, filters, isMyTicketsScope],
    queryFn: async () => {
      if (!isMyTicketsScope) {
  ...
  const pageTitle =
    scope === 'my' ? 'My Tickets' : isAdmin || isTechnician ? 'All Tickets' : 'My Tickets'
