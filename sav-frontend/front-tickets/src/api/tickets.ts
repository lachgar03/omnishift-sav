...
export const ticketsApi = {
  ...
  getStatistics: async (): Promise<TicketStatsResponse> => {
    const { data } = await axiosInstance.get(API_ENDPOINTS.TICKETS_STATISTICS)
    return data
  },

  assignTeam: async (id: string | number, request: AssignTeamRequest): Promise<TicketResponse> => {
    const { data } = await axiosInstance.patch(
      `${API_ENDPOINTS.TICKETS}/${id}/assign-team`,
      request,
    )
    return data
  },

  assignUser: async (id: string | number, request: AssignUserRequest): Promise<TicketResponse> => {
    const { data } = await axiosInstance.patch(
      `${API_ENDPOINTS.TICKETS}/${id}/assign-user`,
      request,
    )
    return data
  },
...
export const ticketAttachmentsApi = {
  ...
  download: async (ticketId: string | number, attachmentId: string | number): Promise<Blob> => {
    const url = API_ENDPOINTS.TICKET_ATTACHMENTS_DOWNLOAD.replace(
      '{ticketId}',
      String(ticketId),
